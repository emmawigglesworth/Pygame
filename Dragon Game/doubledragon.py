...
class Player(pygame.sprite.Sprite):
    ...
    def update(self, rightimg, leftimg, attackimgright, attackimgleft, jumptime):#, attackimg)
        ...
        if self.attack:
            if self.image == rightimg:
                self.image = attackimgright
            if self.image == leftimg:
                self.image = attackimgleft
            self.attack = False
        else:
            if self.image == attackimgright:
                self.image = rightimg
            if self.image == attackimgleft:
                self.image = leftimg
        ...
        if self.isHit:
            self.health -= 1
            self.isHit = False
            #self.player.image = hurtimg FUCK THIS FUCKING L/R BULLSHIT
...
class Opps(pygame.sprite.Sprite):
    ...
    #GOING TO BE BASED ON MAINPLAYER'S POSITION
        ...
        if self.attack:
            if self.image == rightimg:
                self.image = attackimgright #right
            elif self.image == leftimg:
                self.image = attackimgleft #left
        else:
            if self.image == attackimgright: #right
                self.image = rightimg
            elif self.image == attackimgleft: #left
                self.image = leftimg
    ...
    def run_logic(self):
        ...
        for anopp in self.opps:
            ...
            if attack_posy and attack_posx:
                attack_stall = time.time() - anopp.attackTime
                if attack_stall >= anopp.stallTime:
                    anopp.attack = True
